 ...
 * Ideally, it is set to run every day at a preset time to ensure that the events are
 ...
 * 
 * DISCLAIMER: To have this class meet its requirements, we would need to create a separate heroku
 * application for it. As such, this is simply a proof of concept and code showcase.
 */
public class EventManager {
	 ...
	 */
	private static void updateAllEventsStatus() {
		DAL dal = DAL.getInstance();

		while (true) {
			if (LocalTime.now().equals(LocalTime.of(6, 0, 0))) {
				...
				dal.updateEventStatus("active", current, future);
				dal.updateEventStatus("inactive", distantPast, current);
			}
		}
	}
